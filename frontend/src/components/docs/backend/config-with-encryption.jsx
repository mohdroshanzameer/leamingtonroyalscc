/**
 * ============================================================================
 * BACKEND CONFIGURATION WITH PASSWORD ENCRYPTION
 * ============================================================================
 * Enhanced config.js with database password decryption support
 */

require('dotenv').config();
const { getPassword } = require('./db-password-encryption');
...
const config = {
  ...
  // ============================================================================
  // DATABASE CONFIGURATION - WITH PASSWORD DECRYPTION
  // ============================================================================
  database: {
    ...
    // Automatically decrypt password if encrypted
    get password() {
      return getPassword(process.env.DB_PASSWORD);
    },
    ...
    // Encryption settings
    encryption: {
      enabled: process.env.DB_PASSWORD_ENCRYPTED === 'true',
...
function validateConfig() {
  const required = [
    { key: 'DB_PASSWORD', value: process.env.DB_PASSWORD, message: 'Database password is required' },
    { key: 'JWT_SECRET', value: config.auth.jwt.secret, message: 'JWT secret is required' },
  ];

  // Validate encryption key if password encryption is enabled
  if (config.database.encryption.enabled && !process.env.DB_ENCRYPTION_KEY) {
    required.push({
      key: 'DB_ENCRYPTION_KEY',
      value: null,
      message: 'Encryption key required when DB_PASSWORD_ENCRYPTED=true',
    });
  }
  ...
  if (config.server.https.enabled) {
    required.push(
      { key: 'HTTPS_KEY_PATH', value: config.server.https.keyPath, message: 'HTTPS key path required' },
      { key: 'HTTPS_CERT_PATH', value: config.server.https.certPath, message: 'HTTPS cert path required' }
    );
  }
  ...
  // Validate password can be decrypted
  try {
    const password = config.database.password;
    if (!password) {
      throw new Error('Database password is empty after decryption');
    }
  } catch (error) {
    console.error('❌ Database password error:', error.message);
    process.exit(1);
  }
}
